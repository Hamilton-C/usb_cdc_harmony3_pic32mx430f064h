...
)
{
    ...
    switch(event)
    {
        ...
        case USB_DEVICE_EVENT_SOF:
            ...
            if (SWITCH_Get() == 1)
            {
                if (cdcData.ignoreSwitch)
                {
                    break;
                }
                if (cdcData.isDebounceOngoing)
                {
                    cdcData.switchDebounceCounter++;
                    if (cdcData.switchDebounceCounter >= SWITCH_DEBOUNCE_COUNT)
                    {
                        cdcData.isDebounceOngoing = false;
                        cdcData.switchWasPressed = true;
                        cdcData.ignoreSwitch = true;
                    }
                }
                else
                {
                    cdcData.isDebounceOngoing = true;
                    cdcData.switchDebounceCounter = 0;
                }
            }
            else
            {
                cdcData.ignoreSwitch = false;
            }
            break;
